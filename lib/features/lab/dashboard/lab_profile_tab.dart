...
class LabProfileTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return profileAsync.when(
      ...
      data: (profile) {
        if (profile == null) return const Center(child: Text('No Lab Profile Found'));
        ...
        return Column(children: [
          Container(
            ...
            child: Column(children: [
              ...
              Text('Reg: ${profile.registrationNumber ?? 'N/A'}', style: const TextStyle(color: Colors.white70, fontSize: 13, letterSpacing: 0.5)),
              const SizedBox(height: 24),
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                _heroStat('${profile.testTypes.length}', 'Test Types'),
                Container(width: 1, height: 30, color: Colors.white24),
                _heroStat('4.9 ★', 'Rating'),
                Container(width: 1, height: 30, color: Colors.white24),
                _heroStat('Active', 'Status'),
              ]),
            ]),
          ),
          Expanded(
            child: ListView(
              ...
              children: [
                _tileSection('Account Details', [
                  _tile(Icons.location_on_outlined, 'Address', profile.address ?? 'No address provided', const Color(0xFF2C6BFF)),
                  _tile(Icons.phone_outlined, 'City', profile.city ?? 'No city provided', const Color(0xFF10B981)),
                  _tile(Icons.verified_user_outlined, 'Verification', profile.verificationStatus.name.toUpperCase(), const Color(0xFFF59E0B)),
                ]),
                const SizedBox(height: 24),
